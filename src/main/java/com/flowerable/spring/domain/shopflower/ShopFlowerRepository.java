...
import com.flowerable.spring.application.shopflower.dto.ShopFlowerOrderCountDto;
...
public interface ShopFlowerRepository extends JpaRepository<ShopFlower, Long> {
    ...
    @Query("""
    SELECT new com.flowerable.spring.application.shopflower.dto.ShopFlowerOrderCountDto(
            f.name,
            COUNT(DISTINCT o.id)
        )
        FROM OrderItem oi
