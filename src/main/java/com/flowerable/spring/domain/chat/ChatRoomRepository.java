...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    ...
    @Query("""
    select new com.flowerable.spring.application.chat.dto.ChatRoomListRes(
        cr.id,
        cr.userId,
        cr.shopId,
        s.shopName,
        ...
        (
            ...
            where cm.chatRoom.id = cr.id
              and cm.senderType = com.flowerable.spring.domain.chat.SenderType.SHOP
              and cm.isRead = false
        )
    )
    from ChatRoom cr
...
""")
    ...
    select new com.flowerable.spring.application.chat.dto.ChatRoomListRes(
        ...
        (
            ...
            from ChatMessage cm
            where cm.chatRoom.id = cr.id
              and cm.senderType = com.flowerable.spring.domain.chat.SenderType.USER
              and cm.isRead = false
        )
    )
    from ChatRoom cr
